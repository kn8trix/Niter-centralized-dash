...
header. The `core` test suite was reorganised into a `tests/` package so
`python manage.py test core.tests.test_navigation` is the canonical template
check.

### Changes

**Base template (`templates/base.html`):**
- Added `{% block extra_head %}` (page-specific stylesheets + meta) in the
  `<head>`, `{% block body_class %}` on the `<body>` (page CSS like
  `body.transport` keeps working), and a global **FontAwesome** CDN link
  (the converted pages rely on `fa-*` icons).
- Added a **Settings** link (gear icon) to the sidebar nav, completing the
  menu: Dashboard · Academic & Notes · Official Notices · Transport ·
  Meals · Medical · Clubs & Events · Settings.

**Templates converted from standalone to `{% extends "base.html" %}`**
(removed `{% include 'partials/topbar.html' %}` and the top-pill header):
- `templates/dashboard/home.html`
- `templates/research_ai.html`
- `templates/academic/notes.html` (the sidebar's "Academic & Notes" destination)
- `templates/notices/notices.html`
...
Each one now sets `title` / `header` / `body_class` and keeps its own
...
- `core/tests.py` → **`core/tests/` package** (`__init__.py` holds the
  existing suite; the package keeps full-suite discovery working).
- New `core/tests/test_navigation.py` — `UnifiedSidebarLayoutTest`: every
  student subpage renders 200 with `data-region="sidebar"`, the "Niter Hub"
  brand, all eight sidebar links, its page stylesheet, and **no** topbar
  markers (`data-component="topbar"`, `avatar-btn`, `profile-popover`);
  also covers the notes engine and the authenticated `/settings/` page.
- Removed the obsolete `UnifiedHeaderTest` (top-pill assertions);
  `ProfilePopoverAuthTest` now exercises the remaining topbar page
  (`/departments/`) instead of `/medical/`.
- Note: `python manage.py test core.tests` loads only the `__init__.py`
  classes; run `core.tests.test_navigation` (or the full suite) to include
